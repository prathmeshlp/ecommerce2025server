...
export const getAdminDashboard = asyncHandler(async (req: Request, res: Response) => {
  ...
  const revenueTrend = await Order.aggregate([
    { $match: { paymentStatus: "completed" } },
    { $group: { _id: { $dateToString: { format: "%Y-%m", date: "$createdAt" } }, total: { $sum: "$total" } } },
    { $sort: { _id: 1 } },
    { $limit: 6 },
  ...
  res.json(new ApiResponse(200, dashboardData, "Admin dashboard data retrieved successfully"));
});
...
export const deleteUser = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json(new ApiResponse(200, { message: "User deleted successfully" }, "User deleted successfully"));
});
...
export const bulkUpdateOrders = asyncHandler(async (req: Request, res: Response) => {
  const { orderIds, action, paymentStatus } = req.body;

  if (action === "update") {
    if (!paymentStatus) {
      throw new ApiError(400, "Payment status required for update", [], "MISSING_PAYMENT_STATUS");
    }
    ...
    res.json(new ApiResponse(200, { message: `${result.modifiedCount} orders updated successfully` }, "Orders updated successfully"));
  } else if (action === "delete") {
    const result = await Order.deleteMany({ _id: { $in: orderIds } });
    res.json(new ApiResponse(200, { message: `${result.deletedCount} orders deleted successfully` }, "Orders deleted successfully"));
  } else {
...
export const deleteOrder = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json(new ApiResponse(200, { message: "Order deleted successfully" }, "Order deleted successfully"));
});
...
export const getUsersAdmin = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json(new ApiResponse(200, responseData, "Users retrieved successfully"));
});
...
export const bulkUpdateProducts = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!productIds || !Array.isArray(productIds) || productIds.length === 0) {
    throw new ApiError(400, "Product IDs are required and must be an array", [], "INVALID_PRODUCT_IDS");
  }
  if (stock === undefined || stock < 0 || isNaN(stock)) {
    throw new ApiError(400, "Stock must be a non-negative number", [], "INVALID_STOCK");
  }
  ...
  res.json(new ApiResponse(200, { message: `${result.modifiedCount} products updated successfully` }, "Products updated successfully"));
});
...
export const getDiscounts = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json(new ApiResponse(200, responseData, "Discounts retrieved successfully"));
});
...
export const createDiscount = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (applicableProducts) {
    ...
    if (validProducts.length !== applicableProducts.length) {
      throw new ApiError(400, "One or more product IDs are invalid", [], "INVALID_PRODUCT_IDS");
    }
  }
...
export const deleteDiscount = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json(new ApiResponse(200, { message: "Discount deleted successfully" }, "Discount deleted successfully"));
});

export const bulkUpdateDiscounts = asyncHandler(async (req: Request, res: Response) => {
  const { discountIds, isActive } = req.body;

  if (!discountIds || !Array.isArray(discountIds) || discountIds.length === 0) {
    throw new ApiError(400, "Discount IDs are required and must be an array", [], "INVALID_DISCOUNT_IDS");
  }
  if (typeof isActive !== "boolean") {
    throw new ApiError(400, "isActive must be a boolean", [], "INVALID_IS_ACTIVE");
  }
  ...
  res.json(new ApiResponse(200, { message: `${result.modifiedCount} discounts updated successfully` }, "Discounts updated successfully"));
});